...
class Test{
	...
	public function __construct($ip,$nums){
		$time = date('Ymd-H.i.s');
		...
		$this->ip_array=$this->get_ip_array($ip,$nums);
	}	
	...
	public function getVideoInfo(){
		foreach($this->ip_array as $ip){
			...
			curl_close($curl);
			$this->vencsize[$ip] = $video_array['vencsize'];
			$this->codec[$ip]    = $video_array['codec'];
		}	
			
	}
	/************************根据最大分辨率来设置主码流分辨率*************************/
	public function getVideoSize(){
		switch ($this->vencsize["$this->ip"]){
			case 14:
				$this->array0["$this->ip"]=array(2,3,5,7,8,9,14);
				break;			
			case 13:
				$this->array0["$this->ip"]=array(2,3,5,7,8,9,12,13);
				break;
			case 12:
				$this->array0["$this->ip"]=array(2,3,5,7,8,9,12);
				break;
			case 9:
				$this->array0["$this->ip"]=array(2,3,5,7,8,9);
				break;
			case 5:
				$this->array0["$this->ip"]=array(2,3,5,7,8);
				break;
			case 8:
				$this->array0["$this->ip"]=array(2,3,7,8);
				break;
			case 3:
				$this->array0["$this->ip"]=array(2,3,7);
				break;
			default:
				echo "Error Video Size\r\n";
				exit;
		}
		$this->size0["$this->ip"]=$this->array0[$this->ip][array_rand($this->array0[$this->ip])];
		
	}	
	/*****************根据压缩格式来设置当前压缩格式********************/
	public function getCodec(){
		switch($this->codec[$this->ip]){
			case 2:
				$this->codec0[$this->ip] = 2;
				$this->codec1[$this->ip] = 2;
				break;
			case 7:
				/*
                $array=array(2,7);
				$this->codec0[$this->ip] = $array[array_rand($array)];
				$this->codec1[$this->ip] = $array[array_rand($array)];
                */
                $this->codec0[$this->ip] = 7;
				$this->codec1[$this->ip] = 7;
				break;
		}	
	}
		...
		while(true){
			...
			foreach($this->ip_array as $ip){
				...
				if($this->model=='v'){
					...
					/************设置码流*************/
					if($this->size0["$this->ip"]==7||$this->size0["$this->ip"]==2){
                    ...
                    $this->enable1=1;
					/*********设置子码流分辨率******/
					if($this->size0["$this->ip"] == 7){
						$this->array1=array(1,6);	
					}elseif($this->size0["$this->ip"] == 2){
					...
					$this->url="http://$this->ip/?jcpcmd=devvecfg%20-act%20set%20-id%200%20-enable%201%20-vencsize%20{$this->size0[$ip]}%20-fps%20$this->fps0%20-bps%20$this->bps0%20-gop%20$this->gop0%20-fixbps%20$this->fixbps0%20-codec%20{$this->codec0[$this->ip]}%20-id%201%20-enable%20$this->enable1%20-vencsize%20$this->size1%20-fps%20$this->fps1%20-bps%20$this->bps1%20-gop%20$this->gop1%20-fixbps%20$this->fixbps1%20-codec%20{$this->codec1[$this->ip]}";
				}
				/*****************镜像测试****************/
				if($this->model=='r'){
				...
				if($this->model=='n'){
					...
					$network_segment_array=array(202,203);
					...
					$this->new_ip_array[]=$this->ethip;
					echo "set ".$this->ethip.' -> '.$this->ip."\r\n";
				}
				/*****************字幕测试****************/
				if($this->model=='o'){
                ...
                if($this->model=='ircut'){
                    if($i%2==0){
                        $this->url="http://$this->ip/?jcpcmd=ircfg%20-act%20set%20-irswitchmode%201%20-turnonlux%200%20-webturnonlux%20100";
                    }else{
                        $this->url="http://$this->ip/?jcpcmd=ircfg%20-act%20set%20-irswitchmode%201%20-turnonlux%200%20-webturnonlux%201";
                    }
                }
				/****************字幕测试****************/
				if($this->model=='c'){
				...
				if($this->model=='light'){
					if($this->openledlux==100){
						$this->openledlux=1;
					}else{
						$this->openledlux=100;
					}
					$this->url="http://$this->ip/?jcpcmd=ircfg%20-act%20set%20-openledlux%20$this->openledlux";
				}
			...
			if($this->model=='n'){
				$this->ip_array=$this->new_ip_array;
				unset($this->new_ip_array);
			}
			$i++;
...
/******************************************************************************/
$opt=getopt('i:m:t:n:');	
if(!empty($opt['i'])&&!empty($opt['m'])&&!empty($opt['t'])&&!$opt['t']<1&&!empty($opt['n'])){
	$test=new Test($opt['i'],$opt['n']);
	$test->model=$opt['m'];
	$test->sleeptime=$opt['t'];
	$ip_array=$test->ip_array;
	$not_online_dev_nums = 0;
	echo "\r\n";
	for($l = 0;$l < count($ip_array);$l++){//判断是否都在线
		if(!$test -> get_version($ip_array[$l])){
			$not_online_dev_nums++;
	...
	echo "\r\n\r\n\r\nUsage:\r\n\r\n";
	echo "test_mulit.php -i [ip address] -m [model] -t [time] -n [quantity of device]\r\n\r\n";
	echo "-i   IP Address<must>\r\n\r\n";
	echo "-m   Running model<must>\r\n\r\n     [v]--Video Test\r\n\r\n     [a]--Audio Test\r\n\r\n     [r]--Inversion Test\r\n\r\n     [p]--Picture Test\r\n\r\n     [h]--height\r\n\r\n     [e]--extension\r\n\r\n     [i]--Infrared Test\r\n\r\n     [n]--Internet Test\r\n\r\n     [o]--OSD_Name Test\r\n\r\n     [c]--OSD_Osd Test\r\n\r\n     [s]--hidden test\r\n\r\n     [w]--alarm test\r\n\r\n     [y]--OSD_Color Test\r\n\r\n     [reboot]--Reboot Test\r\n\r\n     [w]--Alarm Simulate Test\r\n\r\n";
	echo "-t   run time<must>\r\n\r\n";
	echo "-n   quantity of device<must>\r\n\r\n";
}