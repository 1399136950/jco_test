...
class UpgradeTest{
	...
	public function __construct(){//对象实例化时，自动识别升级包文件
        ...
        while(true){
            if (file_exists($this->root_path.$this->log_dir.'/'.$this->log_file)){
                $this->log_file = $this->log_file.'.1';
            }else{
                break;
            }
        }
	...
	public function get_update_progressbar($IP){//获取升级进度
        ...
        for($i=0;$i<5;$i++){
            ...
            if($output){
                ...
                $this->write_upgrade_log("[get_update_progressbar] Err,Try Again");
                //continue;
            }
        }
	...
	public function start_upgrade_by_tgzs($IP,$FILE){//多个升级包，多个IP升级测试
		...
		if(count($IP) == 1){
			...
			while(true){
                ...
                if ($i%2 == 0){
                    ...
                    $ret=$this->upload_file_by_curl($url, $FILE[$tttt]);
                }
                if(!$ret){
                    echo "sleep 40\r\n";
                    sleep(40);
				...
				if($this->is_update($IP)){
                    $this->write_upgrade_log('[start_upgrade_by_tgzs]Upload Finshed,Sleep('.$this->sleep_time.')');
                    sleep($this->sleep_time);
                }else{
                    echo ",err\r\n";
                    $this->write_upgrade_log('[start_upgrade_by_tgzs]progressbar error');
                    exit(0);
                }
		...
		//多个IP升级
            ...
            $this->p = new Progessbar(30);//实例化进度条类
            $is_loaded_pthread = extension_loaded('pthreads');
            if($is_loaded_pthread){
					require_once('ThreadForUpgrade.php');//导入多线程类
            }
			while(true){
				...
				if($is_loaded_pthread){
					...
					$t1 = microtime(true);
					for($i = 0;$i < count($IP);$i++){
						//$this->write_upgrade_log('[start_upgrade_by_tgzs]sleep(1)');
						//sleep(1);//每次上传文件间隔2秒
                        //usleep(50000);
                        ...
                        if ( $j%2 == 0 ){
                            ...
                            $url = 'http://'.$IP[$i].$this->upgrade_url;
                            $this->upload_file_by_curl($url, $FILE[$tttt]);
                        }
					}
					$t2 = microtime(true);
					echo "It's take ".round($t2-$t1,3)." secs\r\n";
                ...
                $this->showProgess($this->sleep_time_nums);//sleep
			...
			for($i=1;$i<=$this->test_nums;$i++){
				...
				for($require_nums = 0;$require_nums <= $this->wait_result_nums;$require_nums++){
					if($this->get_version($ip)){
						...
						echo "Get Test Data......It takes $time seconds\r\n\r\n";
	...
	public function get_upload_file(){//返回根目录升级包信息
        echo "$this->root_path$this->upgrade_file_dir\r\n";
        if (file_exists($this->root_path.$this->upgrade_file_dir)){
            ...
            echo "upgrade file folder not exists,please check!!!\r\n";
            exit(0);
        }
	}
