...
class WlxTest(TestBase, unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        try:
            ...
            sleep(1)
            self.user_info=[['admin','admin'],['admin','admin12345'],['admin','admin123456'],['admin','123456'],['admin','12345'],['admin','888888'],['user','user'],['test','test']]#预存放的账户密码
            try:
            ...
            try:
                WebDriverWait(driver,20,0.5).until(EC.presence_of_element_located((By.ID,"deviceBtn")))
            except:
                print('no')
            else:
                sleep(0.5)
    ...
    def model_init(self,model):
        try:
            ...
            self.driver.find_element_by_id("deviceBtn").send_keys(Keys.ENTER)
            self.driver.switch_to.frame("layerWindow")
            self.driver.switch_to.frame("layui-layer-iframe"+str(WlxTest.index))
            self.driver.find_element_by_id(model)
        except:
            pass
        else:
            try:
                ...
                try:
                    ...
                    self.driver.find_element_by_id("dww-menu").find_element_by_tag_name("div")
                except:
                    print(2)
                else:
                    try:
    ...
    def before_model_init(self):
        try:
            self.driver.switch_to.default_content()
            self.driver.switch_to.frame("layerWindow")
            self.driver.switch_to.frame("layui-layer-iframe"+str(WlxTest.index))
            self.driver.find_element_by_css_selector("a[class='layui-layer-ico layui-layer-close layui-layer-close1']").click()
            self.MyClickById('img_exit')
            WlxTest.index+=1
        except:
            WlxTest.index+=1
            print('not click')
    ...
    def test_sysset(self):
        sleep(3)
        ul=self.model_init("img_SYSTEM_SET")
        for li in ul:
            if(len(li.text)>0):
    ...
    def test_netset(self):
        self.before_model_init()
        ul=self.model_init("img_NETWORKSET")
        for li in ul:
            if(len(li.text)>0):
    ...
    def test_videoimage(self):
        self.before_model_init()
        ul=self.model_init("img_VIDEOIMAGE")
        for li in ul:
            if(len(li.text)>0):
                if(li.text in self.title["SUBTITLE_OVERLAY"]):
                ...
                elif(li.text in self.title["VIDEOSETTING"]):
                    #js='document.getElementById("tab_colorsetting").click()'
                    #self.driver.execute_script(js)
                    li.click()
                    try:
                        WebDriverWait(self.driver,20,0.5).until(EC.presence_of_element_located((By.ID,"tdBright")))
                    except:
                        pass
                    else:
                        print('        [图像设定]:')
    ...
    def test_videoparam(self):
        self.before_model_init()
        ul=self.model_init("img_VIDEOPARAM")
        for li in ul:
            if(len(li.text)>0):
                
                if(li.text in self.title["AUDIOSET"]):
    ...
    def test_alarmset(self):
        self.before_model_init()
        ul=self.model_init("img_ALARM_SET")
        for li in ul:
            if(len(li.text)>0):
                li.click()
                sleep(1)
                ...
                if(li.text in self.title["monitor_cross_alarm"]):#越界
                    ...
                    #WlxTest.index+=1
                    '''
                    self.MyClickById('m_select_cross')
                    self.driver.switch_to.frame("layui-layer-iframe1")
    ...
    def test_platset(self):
        self.before_model_init()
        ul=self.model_init("img_PLATFORM_SET")
        print('        [平台设置]:')
        if ul=='EMPTY':
            print('EMPTY')
        else:
